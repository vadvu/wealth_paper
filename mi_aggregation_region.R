...
all.me.nvc <- read.csv("imp_models/all.me.nvc_region_final.csv")
...
all.me.rev <- read.csv("imp_models/all.me.rev_region_final.csv")
...
ggsave("plots/me.comp.MI_region_final.png", dpi = 600, device = "png", units = "cm", width = 25, height = 12.5)


##### aggregated tables 
### navco
all.coef.nvc <- read.csv("imp_models/all.coef.nvc_region_final.csv")
...
### bess
all.coef.rev <- read.csv("imp_models/all.coef.rev_region_final.csv")
...
#### linear navco
all.coef.nvc.lin <- read.csv("imp_models/all.coef.nvc.linear_region_final.csv")
...
#### linear bess 
all.coef.rev.lin <- read.csv("imp_models/all.coef.rev.linear_region_final.csv")
...
tab <- tab.reg(x)
tab
save_as_docx(tab, path = "tables/tab1_region_final.docx")


####### prediction
### navco
all.pred.nvc <- read.csv("imp_models/all.pred.nvc_region_final.csv")
...
pr.imp.nvc <- ggplot(all.pred.nvc.11, aes(x = x))+
  geom_ribbon(data = pred.nvc, aes(ymin = ifelse(y-1.96*se<0,0,y-1.96*se), ymax = y+1.96*se), 
              alpha = 0.5, size = 0.3, fill = "pink")+
...
#### bess
all.pred.rev <- read.csv("imp_models/all.pred.rev_region_final.csv")
...
pr.imp.rev <- ggplot(all.pred.rev.11, aes(x = x))+
  geom_ribbon(data = pred.rev, aes(ymin = ifelse(y-1.96*se<0,0,y-1.96*se), ymax = y+1.96*se), 
              alpha = 0.5, size = 0.3, fill = "pink")+
  geom_ribbon(aes(ymin = ifelse(y.mean - 1.96*sqrt(se2)<0,0,y.mean - 1.96*sqrt(se2)), 
                  ymax = y.mean + 1.96*sqrt(se2)
  ), 
...
ggsave("plots/me.comp.pred_region_final.png", dpi = 600, device = "png", units = "cm", width = 25, height = 12.5)
